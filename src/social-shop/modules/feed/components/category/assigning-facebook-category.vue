  ...
  <div v-if="status" class="category-rule">
    <div class="box-assign-category m-b-20">
      <div class="d-flex header-function-assign">
        ...
        <div class="col col-5 custom-space-header">Facebook Product Categories</div>
      </div>
        ...
        <div v-else key="loading-success">
          <template v-if="categories.collection_categories.length > 0">
            <div v-for="(row, index) in categories.collection_categories" :key="`item_${index}`" class="section-assign m-b-20 d-flex">
              ...
              <div class="col col-md-5">
                <a-dropdown
                  :class="{ 'force-validate-category': row.validate_category || row.validate_force == false }"
                  transitionName="none"
                ...
                <div class="validate-category" v-else-if="row.validate_force == false"> Please change the collections with Google Categories to Facebook Categories to match the platform's requirement.</div>
              </div>
...
import google_product_category from '@/config/facebook-product-category.json'
import 'vue-virtual-scroller/dist/vue-virtual-scroller.css'
const LOAD_THRESHOLD_PX = 300
import { RecycleScroller } from 'vue-virtual-scroller'
export default {
  name: 'CategoryFacebook',
  props :{
    value :{
      type : Object
    },
  ...
  data() {
    return {
      data: google_product_category.slice(0, 200),
      loading: false,
      saving: false,
      search: '',
      arrayItems: [],
  ...
  methods: {
    ...
    visibleChange(row) {
      if (row) {
        this.data = google_product_category.slice(0, 200)
      } else {
    ...
    chooseCategory(selectedCategory, row) {
      this.is_modify = true
      row.google_product_category = selectedCategory
      row.validate_force = true
    },
    handleSearch(e) {
    ...
    handleInfiniteOnLoad() {
      ...
      setTimeout(() => {
        ...
        var temp = data.concat(google_product_category.slice(this.data.length, 200 + this.data.length))
        this.data = temp
        this.loading = false
      }, 1000)
    },
    ...
    forceUserUseListFb() {
      //check validate obligate user use list facebook category
      if (this.categories.check_migrate_categories == false) {
        this.categories.collection_categories.forEach((element) => {
          if (element.hasOwnProperty('validate_force') && element.validate_force == true) {
            return true
          } else {
            element['validate_force'] = false
    ...
    validateField() {
      ...
      this.categories.collection_categories.forEach((element) => {
        ...
        if (
          (element.internal_id === '' || this.forceUserUseListFb() || element['validate_category'] || element['validate_duplicated_collection']) &&
          this.validated
        ) {
          this.validated = false
        }
