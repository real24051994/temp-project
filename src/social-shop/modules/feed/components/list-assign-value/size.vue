<template>
  <div>
    <input v-model="valueField" placeholder="Size" type="text" class="ant-input m-t-5" :class="{ 'form-group--error': $v.valueField.$invalid && form_state }" />
    <div v-if="form_state && $v.valueField.$invalid">
      <div v-if="!$v.valueField.required" class="d-flex m-t-10">
        ...
        <span class="custom-error-message">Size Required</span>
      </div>
    </div>
  </div>
</template>
