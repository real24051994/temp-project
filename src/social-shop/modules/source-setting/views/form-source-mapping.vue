<template>
	<div class="product-source-mapping">
		<transition name="fade-in" mode="out-in">
			<div class="toolbar-navbar-fixed" v-if="modify">
				<template>
					<a-button class="btn-discard" @click="discard()" :disabled="loading">
						Discard
					</a-button>
					<a-button class="btn-save" @click="onSubmit" :disabled="loading" :loading="loading">
						Save Settings
					</a-button>
				</template>
			</div>
          ...
          Data Type
			...
			<transition name="fade-in" mode="out-in">
				<div v-if="spinning" key="is-loading" class="m-t-10">
					<a-skeleton :loading="spinning" v-for="item in 10" :key="`skeleton_${item}`" active :title="false" :paragraph="{ rows: 1, width: '100%' }" class="m-b-10"></a-skeleton>
				</div>
                ...
                {{ item.data_type }}
							...
							<div class="column-assigned d-flex flex-column">
								<div class="item-mapping" :class="{ 'force-validate-metafields': item.validate_metafields }">
									<vue-mapping-import-contenteditable :list_mapping="listMapping" :array_check_selected.sync="arrayCheckSelected" :modify.sync="modify" v-model="filteredList[index]" />
								</div>
		...
		methods: {
			...
			renderList() {
				...
				return new Promise((resolve, reject) => {
					this.getListImportMapping({ shop_id : this.sourceId })
						.then((res) => {
