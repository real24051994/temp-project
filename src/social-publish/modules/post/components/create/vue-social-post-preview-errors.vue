	...
	<div class="vue-social-preview-error">
		<transition-group name="fade-in" mode="out-in"  tag="ul" >
			<li v-for="(item,index) in data" :key="`error_index_${index}`">
				...
				<template v-else-if="item == 'aspect_ratio'">
					{{ 
						$t('error.aspect_ratio', { 
							min_aspect_ratio : rules.min_aspect_ratio ,
							max_aspect_ratio : rules.max_aspect_ratio 
						}) 
					}}
				</template>
				<template v-else-if="item == 'max_image'">
					{{ $t('error.max_image', { max_image : rules.max_image }) }}
				</template>
			</li>
		</transition-group>
	</div>
