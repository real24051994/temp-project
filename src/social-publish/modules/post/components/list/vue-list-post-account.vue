<template>
	<div class="vue-list-social-account" :class="{'is-collapsed' : aside_collapse }">
		...
		<div class="vue-list-social-account-footer" >
			<router-link  :to="{ name: 'ManageAccount'}" id="vue-list-post-accounts-btn-trigger" type="button" class="btn-bold btn-block ant-btn btn-manage-accounts-in-list-socials"  data-component="Manage Accounts">
...
export default {
	...
	computed:{
		...
		showAllOption(){
			return this.status == 'draft' || this.status == 'error'
		},
		loading_rows(){
	...
	methods:{
		...
		routerLinkActive(social_id ){
			var id = this.$route.query && this.$route.query.hasOwnProperty('social_id') ? this.$route.query.social_id : ''
			if( id ){
				return id == social_id
			}else{
				return social_id == this.data[0].id
			}
		},
		handleManageAccount(){
			this.redirect({
