						...
						<div  v-else key="is-loading-success">
										...
										<div class="list-info-inline">
											<div class="item-info-inline">
												...
												<label class="item-template-image">
													<img v-if="form.background_id" :src="form.background_id" alt="">
													<input type="file" hidden class="hide" @change="onChange($event, 'background_id')">
												</label>
											</div>
											<div class="item-info-inline m-l-15">
												<div>Sample</div>
												<label class="item-template-image">
													<img v-if="form.sample_id" :src="form.sample_id" alt="">
													<input type="file" hidden class="hide" @change="onChange($event, 'sample_id')">
												</label>
											</div>
							...
							<template v-else>
								<div class="p-t-50 p-b-50 text-center">
									<h1>Not Found</h1>
									<router-link class="ant-btn ant-btn-primary" :to="{ name : 'Autopilottemplate'}">
										List Tempaltes
									</router-link>
								</div>
							</template>
...
export default {
	...
	methods:{
		...mapActions({
			updateDiscountTemplate : 'utils/updateDiscountTemplate',
			getDetailDiscountTemplate : 'utils/getDetailDiscountTemplate'
		}),
		toBase64(file){
		...
		updatePreview(){
			let { width, height , background_id , content } =  this.form
			if( background_id ){
