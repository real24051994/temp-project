<template>
	<div class="emoji-picker">
		<div class="emoji-dropdown" ref="emojiDropdown">
			<div class="header">
				<template  v-for="(val,key) in categories">
					<span :key="key" class="pointer" :class="{ active: key === scrolledTo }"  @click="scrollToCategory(key)" v-html="val">
					</span>
				</template>
			</div>
			<div  class="emojis" ref="emojis">
				<template  v-for="(items, category) in emojis">
					<div :key="category" >
						<div class="category" :ref="getCategoryRef(category)">
							{{category}}
						</div>
						<div class="emoji-list">
							<span v-for="(item, index) in items" class="emoji pointer" :key="index" :title="item.aliases" @click="selectEmoji(item)">
								{{ item.emoji }}
							</span>
						</div>
					</div>
...
import {categories, searchSVG, emojiInvokerOpen, emojiInvokerClose} from './svg.js';
...
export default {
  	...
  	methods: {
  		...
  		selectEmoji(emojiObj) {
  			this.$emit('on-select', emojiObj.emoji);
  		...
  		getToneHands() {
  			return skinToneNames.map(item => {
  				return {
  					...item,
  					...{
  						emoji: skinTone('✋', skinTone[item.name])
  					}
  				};
